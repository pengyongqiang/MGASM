...
def bert_uncased_embed(docs, batch_size, dim):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = BertForSequenceClassification.from_pretrained(path_conf.bert_base, num_labels=dim).to(device)
    tokenizer = BertTokenizer.from_pretrained(path_conf.bert_base)
