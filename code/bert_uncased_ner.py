...
def bert_uncased_ner(docs, batch_size, dim):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = BertForSequenceClassification.from_pretrained(path_conf.bert_base_ner, num_labels=dim, ignore_mismatched_sizes=True).to(device)
    tokenizer = BertTokenizer.from_pretrained(path_conf.bert_base_ner)
