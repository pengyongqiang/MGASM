...
from embedding_mathod import char_embedding, word_embedding, lda_embedding, electra_embed, graph_embed, bert_embed


def embed_wd():
    attrs = pickle.load(open('../dataset/wd/attrs', 'rb'))
    g1, g2 = pickle.load(open('../dataset/wd/networks', 'rb'))
    print(time.ctime(), 'Size of the social user relationship struc:', len(g1), len(g2))
    user_list = []
    for i in range(len(attrs)):
        user_list.append(User(attrs[i], 'wd'))

    dim_list = [50, 100, 150, 200, 250, 300]
    # dim_list = [100]
    for dim in dim_list:
        emb_path = f'../emb/wd/emb_dim_{dim}/'
        if not os.path.exists(emb_path): os.makedirs(emb_path)
        print(f"正在进行嵌入，维度：{dim}")

        print("正在嵌入字符粒度属性...")
        char_emb = char_embedding(user_list, 'char', dim, 'cn')
        np.save(emb_path + "char.npy", char_emb)
        print("字符粒度属性嵌入完成。")

        print("正在嵌入标签粒度属性...")
        label_emb = char_embedding(user_list, 'label', dim, 'cn')
        np.save(emb_path + "label.npy", label_emb)
        print("标签粒度属性嵌入完成。")

        print("正在嵌入下词粒度属性...")
        word_emb = word_embedding(user_list, 'word', dim, g1, g2, 'cn')
        np.save(emb_path + "word.npy", word_emb)
        print("词粒度属性嵌入完成。")

        print("正在嵌入文章...")
        article_plm = electra_embed(user_list, 1024, dim).numpy()
        article_lda = lda_embedding(user_list, dim, 'cn')
