...
if __name__ == '__main__':
    ...
    folder_path = '../data/wd/models/classifier'
    ...
    for score_type in ['hit_precision']:
        ...
        for dim in [100]:
            emb_path = '../emb/emb_wd_seed_0_dim_{}/emb'.format(dim)
            emb_c, emb_l, emb_w, emb_a_pre, emb_a_lda, emb_g = pickle.load(open(emb_path, 'rb'))
            emb_default = np.concatenate((emb_c, emb_l, emb_w, emb_a_pre), axis=-1)
            emb_no_l = np.concatenate((emb_c, emb_w, emb_a_pre), axis=-1)
            emb_lda = np.concatenate((emb_c, emb_l, emb_w, emb_a_lda), axis=-1)
            for model in range(3):
                emb = [emb_default, emb_no_l, emb_lda][model]
                ...
                for is_correct in [True, False]:
                    ...
                    for n_train in [200]:
                        psearch_fun = partial(psearch, n_train, emb, granularity_num,score_type, is_correct, 'wd')
                        score_10 = []
                        for seed in list(range(1)):
                            result = psearch_fun(seed)
                            score_10.append(result)
        ...
        # df.to_excel("../result/MGASM_WD_{}.xlsx".format(score_type), index=False)
